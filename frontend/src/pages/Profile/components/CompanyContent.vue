<template>
  <v-container fluid class="h-100 d-flex flex-column ga-2" v-if="!auth.isCustomer()">
      ...
      <template #content>
        ...
        >
          <div class="d-flex ga-3 mt-2">
            <v-chip rounded density="compact" color="orange"> {{ `${user?.role}` }}</v-chip>
            <v-chip rounded density="compact" color="blue">
              {{ `${store.employee?.title}` }}</v-chip
            >
          </div>
